...
void ShisaDecrypt::big_conversion_alphabet(std::string small, std::string big) {
    std::transform( small.begin(), small.end(), big.begin(), [](char c){ return std::toupper( c ); } );
}

void ShisaDecrypt::small_conversion_alphabet(std::string big, std::string small) {
    std::transform( big.begin(), big.end(), small.begin(), [](char c){ return std::tolower( c ); } );
}

std::string ShisaDecrypt::move_word(std::string word, int move_point) {
...
bool ShisaDecrypt::check_word(std::string word) {
    bool result = false;
...
void ShisaDecrypt::decrypt(std::string file_name) {
